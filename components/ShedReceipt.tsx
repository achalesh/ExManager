...
interface ShedReceiptProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    allocations: any[];
}

export function ShedReceipt({ open, onOpenChange, allocations }: ShedReceiptProps) {
    ...
    const grandTotal = allocations.reduce((sum, a) => sum + (a.price || 0), 0);
    ...
    const handleWhatsAppShare = () => {
        ...
        let message = `*ExManager Receipt (Shed)* \n`;
        ...
        message += `*Allocated Sheds:*\n`;

        allocations.forEach(alloc => {
            message += `- ${alloc.shed.name} (${alloc.shed.dimensions})`;
            message += ` - ₹${alloc.price.toFixed(2)}`;
            message += `\n`;
        });
            ...
            <DialogContent className="sm:max-w-[600px] print:shadow-none print:border-none print:max-w-full print:w-full print:h-screen">
                ...
                <div className="print-content bg-white p-4 rounded-lg">
                    ...
                    <div className="text-center border-b pb-4 mb-4">
                        ...
                        <DialogTitle className="text-xl font-semibold uppercase tracking-wide text-center border-t pt-2 mt-2">Shed Receipt</DialogTitle>
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {allocations.map((alloc) => (
                                    <tr key={alloc.id}>
                                        <td className="py-3">
                                            <div className="font-medium">{alloc.shed.name}</div>
                                            <div className="text-xs text-gray-500">{alloc.shed.description}</div>
                                        </td>
                                        <td className="text-right py-3 text-gray-600">{alloc.shed.dimensions}</td>
                            ...
                            <tfoot className="border-t-2 border-gray-100 font-semibold">
                                <tr>
                                    <td colSpan={2} className="pt-4 text-right pr-4">Grand Total</td>
