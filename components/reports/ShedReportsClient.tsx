...
interface ShedReportsClientProps {
    allocations: any[];
...
export function ShedReportsClient({
    allocations,
    financialSummary,
    eventName,
    totalRevenue
}: ShedReportsClientProps) {
    ...
    const downloadCSV = (type: 'detailed' | 'financial') => {
        ...
        if (type === 'detailed') {
            headers = ['Exhibitor', 'Facia', 'Space', 'Shed Type', 'Dimensions', 'Price'];
            filename = `Shed_Allocation_Detailed_${eventName.replace(/\s+/g, '_')}.csv`;
            ...
            financialSummary.forEach((item: any) => {
                item.allocations.forEach((alloc: any) => {
                    rows.push([
                        ...
                        `"${alloc.shed.name}"`,
                        `"${alloc.shed.dimensions}"`,
                        alloc.price.toFixed(2)
                    ]);
                });
            });

        } else {
            headers = ['Exhibitor', 'Facia', 'Space', 'Total Sheds', 'Total Cost'];
            filename = `Shed_Payment_Summary_${eventName.replace(/\s+/g, '_')}.csv`;
    ...
    return (
        <div className={`space-y-6 ${printMode === 'detailed' ? 'print-detailed-only' : ''} ${printMode === 'financial' ? 'print-financial-only' : ''}`}>
            ...
            <div className="flex justify-between items-end border-b pb-4 mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">Shed Allocation Report</h1>
            ...
            {/* Section 1: Detailed Shed Allocation List */}
            <div className="detailed-section mb-12 break-inside-avoid">
                <div className="flex items-center justify-between mb-4 bg-gray-100 p-2 rounded">
                    <h2 className="text-xl font-bold text-gray-800">
                        1. Detailed Shed Allocation List
                    </h2>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 border text-sm">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left font-bold text-gray-700 uppercase border-r w-1/4">Shed Type</th>
                                <th className="px-4 py-3 text-center font-bold text-gray-700 uppercase border-r">Dimensions</th>
                                    ...
                                    {item.allocations.map((alloc: any, idx: number) => (
                                        <tr key={alloc.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-2 border-r align-top">
                                                {alloc.shed.name}
                                                <div className="text-xs text-gray-500">{alloc.shed.description}</div>
                                            </td>
                                            <td className="px-4 py-2 border-r text-center align-top font-medium text-gray-600">
            ...
            <div className="financial-section break-before-page">
                <div className="flex items-center justify-between mb-4 bg-gray-100 p-2 rounded">
                    <h2 className="text-xl font-bold text-gray-800">
                        2. Payment Details of Allocated Sheds
                    </h2>
                    <div className="flex gap-2 no-print">
                    ...
                    <table className="min-w-full divide-y divide-gray-200 border text-sm">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-center font-bold text-gray-700 uppercase border-r">Total Sheds</th>
